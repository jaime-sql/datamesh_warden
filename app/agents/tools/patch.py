...
class GeminiPatchGenerator:
    ...
    async def generate(
        ...
        prompt = (
            ...
            "BigQuery DDL syntax notes (this project's sandbox has caught real "
            "model hallucinations here before -- follow exactly):\n"
            "- BigQuery has no traditional row-level indexes. Never propose "
            "`CREATE INDEX`. To fix a full-table-scan / OOM / slow-query "
            "problem, add clustering with exactly: "
            "`ALTER TABLE \\`<table>\\` SET OPTIONS (clustering_fields = "
            "['<column>'])`. There is no `ADD CLUSTERING BY` syntax -- it "
            "does not exist and will fail.\n"
            "- Time partitioning cannot be added to an existing table via "
            "`ALTER TABLE ... SET OPTIONS`; that only works at table-creation "
            "time. Do not propose `time_partitioning_field` or "
            "`time_partitioning_type` options on an existing table.\n"
            "- To add a column: `ALTER TABLE \\`<table>\\` ADD COLUMN IF NOT "
            "EXISTS \\`<column>\\` <TYPE>`.\n\n"
